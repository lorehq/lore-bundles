...
3. **No speculation as fact.** Do not present unverified claims using hedging language as if they were findings. Banned patterns: "probably", "I think", "seems like", "appears to be", "likely". If a claim is not verified, label it explicitly as **Unverified** or **Contested** — do not soften it with hedging.
...
1. **Formulate diverse queries** — vary keywords, include year filters
...
Classify every source on discovery. See `references/source-tiers.md` for the full classification guide.
...
| B | ⚠️ | Social media, forums, personal blogs, wikis — flag to user | Twitter/X, Reddit, StackOverflow, Medium, dev.to, Wikipedia |
...
- **With creativity-sampler:** Can be invoked during option generation for fact-based inputs
